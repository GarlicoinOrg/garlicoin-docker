...
generate_dockerfile() {
    ...
    echo "" >> $fn
    echo "RUN echo \"deb http://archive.debian.org/debian/ squeeze main contrib non-free\" >> /etc/apt/sources.list \\" >> $fn
    echo "    echo \"deb-src http://archive.debian.org/debian/ squeeze main contrib non-free\" >> /etc/apt/sources.list" >> $fn
    echo "RUN useradd -r garlicoin    \\" >> $fn
    echo "    && mkdir /garlicoin     \\" >> $fn
    echo "    && apt-get update -y    \\" >> $fn
    echo "    && apt-get install -y   \\" >> $fn
    echo "        --allow-unauthenticated \\" >> $fn
    echo "        git                 \\" >> $fn
